...
const ability_def &get_ability_def(const int ability_id) {
    for (const auto &abil: Ability_List) {
        if (abil.ability == ability_id) {
            return abil;
        }
    }
...
const char *get_ability_name_by_index(const char index) {
    const auto abil = get_ability_def(Curr_abil[index].which);
    return abil.name;
}
...
bool activate_ability() {
    ...
    const unsigned char spc = static_cast<int>(keyin);
    ...
    const unsigned char spc2 = letter_to_index(spc);
    if (Curr_abil[spc2].which == -1) {
        mpr("You can't do that.");
        return false;
    }

    const unsigned char abil_used = spc2;
    ...
    const ability_def abil = get_ability_def(Curr_abil[abil_used].which);
    ...
    switch (abil.ability) {
        ...
        case ABIL_GLAMOUR:
            ...
            cast_glamour(10 + random2(you.experience_level + random2(you.experience_level)));
        ...
        case ABIL_EVOKE_MAPPING: // randarts
            ...
            magic_mapping(
                3 + roll_dice(2, you.skills[SK_EVOCATIONS]),
        ...
        case ABIL_MAPPING: // Gnome + sense surrounds mut
            mpr("You sense your surroundings.");
            magic_mapping(
                3 + roll_dice(2, you.experience_level) + you.mutation[MUT_MAPPING] * 10,
        ...
        case ABIL_BREATHE_STEAM:
            if (you.duration[DUR_BREATH_WEAPON] && Curr_abil[abil_used].which != ABIL_SPIT_ACID) {
                canned_msg(MSG_CANNOT_DO_YET);
                return false;
            } else if (spell_direction(abild, beam) == -1) {
                canned_msg(MSG_OK);
        ...
        case ABIL_THROW_FROST:
            ...
            zapping((Curr_abil[abil_used].which == ABIL_THROW_FLAME
                         ? ZAP_FLAME
                         : ZAP_FROST),
        ...
        case ABIL_MAKHLEB_MINOR_DESTRUCTION:
            ...
            switch (random2(5)) {
                case 0: zapping(ZAP_FLAME, power, beam);
                    break;
                case 1: zapping(ZAP_PAIN, power, beam);
                    break;
                case 2: zapping(ZAP_STONE_ARROW, power, beam);
                    break;
                case 3: zapping(ZAP_ELECTRICITY, power, beam);
                    break;
                case 4: zapping(ZAP_BREATHE_ACID, power / 2, beam);
                    break;
                default: ;
            }
        ...
        case ABIL_MAKHLEB_MAJOR_DESTRUCTION:
            ...
            switch (random2(8)) {
                case 0: zapping(ZAP_FIRE, power, beam);
                    break;
                case 1: zapping(ZAP_FIREBALL, power, beam);
                    break;
                case 2: zapping(ZAP_LIGHTNING, power, beam);
                    break;
                case 3: zapping(ZAP_NEGATIVE_ENERGY, power, beam);
                    break;
                case 4: zapping(ZAP_STICKY_FLAME, power, beam);
                    break;
                case 5: zapping(ZAP_IRON_BOLT, power, beam);
                    break;
                case 6: zapping(ZAP_ORB_OF_ELECTRICITY, power, beam);
                    break;
...
char show_abilities() {
    ...
    for (const auto &talent: Curr_abil) {
    ...
    for (int do_invoke = 0; do_invoke < (can_invoke ? 2 : 1); do_invoke++) {
        ...
        for (loopy = 0; loopy < Curr_abil.size(); loopy++) {
            ...
            if (Curr_abil[loopy].which != ABIL_NON_ABILITY && do_invoke == Curr_abil[loopy].is_invocation) {
                anything++;
                ...
                const ability_def abil = get_ability_def(Curr_abil[loopy].which);
                ...
                const int spell_f = Curr_abil[loopy].fail;
                cprintf((spell_f >= 100)
                            ? "Useless"
                            : (spell_f > 90)
                                  ? "Terrible"
...
bool generate_abilities() {
    ...
    for (auto &[which, fail, is_invocation]: Curr_abil) {
        which = ABIL_NON_ABILITY;
        fail = 100;
        is_invocation = false;
    }
    ...
    if (you.species == SP_NAGA) {
        insert_ability(you.mutation[MUT_BREATHE_POISON] ? ABIL_BREATHE_POISON : ABIL_SPIT_POISON);
    ...
    // still shameless. -- bwr
    for (const auto &abil: Curr_abil) {
        if (abil.which != ABIL_NON_ABILITY) {
            return (true);
        }
    }
...
void set_god_ability_helper(const int abil, const char letter) {
    const int index = letter_to_index(letter);

    bool found = false;
    for (const auto ability_ref: you.ability_letter_table) {
        if (ability_ref == abil) {
            found = true;
            break;
        }
    ...
    // Ability is not already assigned
    if (!found) {
...
void set_god_ability_slots() {
    ...
    for (auto &abil: you.ability_letter_table) {
        if (abil >= ABIL_ZIN_REPEL_UNDEAD && abil <= ABIL_ELYVILON_GREATER_HEALING // is a god ability
    ...
    if (num_abil) {
        for (char i = 0; i < num_abil; i++) {
            const char letter = i + Options.lowercase_invocations ? 'a' : 'A';
            set_god_ability_helper(abil_start + i, letter);
        }
    }
}
...
static int find_ability_slot(const int which_ability) {
    int slot;

    for (slot = 0; slot < you.ability_letter_table.size(); slot++) {
...
static bool insert_ability(const int which_ability) {
